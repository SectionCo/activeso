...
A returned record supports `record.Save(ctx)` and `record.Delete(ctx)`. A manually built or copied record must be attached first with `model.Bind(&record)`; otherwise those methods return `activeso.ErrUnboundRecord`.
...
`model.AutoMigrate(ctx)` explicitly creates missing tables, adds missing nullable columns, and creates unique indexes. It is deliberately additive and safe: it never removes columns or indexes, changes types, or tightens constraints. Supported `activeso` field constraints are `not_null` and `unique`; `unique` is checked before writes for a friendly `ErrUnique` result and enforced by Turso with a unique index.
